# 👋 Hi, I'm Geet Prajapati

<p align="left">
  <img src="https://img.shields.io/badge/Location-Gujarat,%20India-blue" />
  <img src="https://img.shields.io/badge/Role-Software%20Engineer%20%7C%20ML%20Systems-success" />
  <img src="https://img.shields.io/badge/Graduation-May%202026-informational" />
</p>

Passionate cybersecurity student with experience in ethical hacking, QA testing, web development, python development, and game testing. Skilled in identifying vulnerabilities, improving software quality, and building secure, user-friendly applications.

---

## 🧑‍💻 About Me

- 🎓 B.Tech in Computer Engineering (Cyber Security)
- 🔐 Interested in Cybersecurity & Ethical Hacking
- 🧪 Experienced in QA & Manual Testing
- 🎮 Game Tester & Developer Intern
- 🌱 Currently learning advanced security and web technologies
- 💡 Focused on secure and scalable digital solutions

---

## 🛠 Skills

### 🔐 Cybersecurity
- Ethical Hacking
- Network Security
- Security Testing
- Vulnerability Analysis

### 💻 Development
- HTML
- CSS
- JavaScript
- React.js
- Node.js
- MongoDB
- Python

### 🧪 Testing
- QA Testing
- Manual Testing
- Regression Testing
- Stress Testing
- Bug Tracking
- UI/UX Validation

### ⚙ Programming
- Python
- JavaScript
- HTML

---

## 💼 Experience

### 🎮 RedPlum Games — Game Tester
**2023 - 2025**

- Tested games for bugs, glitches, and performance issues
- Improved gameplay experience and UI/UX
- Conducted functional and regression testing

### 🎮 RedPlum Games — Game Tester & Developer Intern
**Jan 2025 - Jun 2025**

- Assisted in game development and debugging
- Collaborated with developers to improve gameplay mechanics
- Performed QA testing and bug reporting

### 🧪 Macrobian Games — Manual Tester
**2025 - Present**

- Performed CRM machine testing
- Conducted website and UI testing
- Tested software stability and hardware behavior

---

## 🚀 Projects

### 🌐 Enhancing Real-Time Noise Pollution Monitoring
- Built a real-time noise monitoring system
- Used React.js, Node.js, and MongoDB
- Added Google Sign-In and 2FA authentication
- Implemented map-based visualization

### 💡 Automatic Street Light using IoT
- Smart street lighting system using IoT
- Automated lighting based on environmental conditions

---

## 🏆 Certifications & Badges

- Cyber Security Lab Building Internship
- Security & Testing Certifications
- Credly Badges

🔗 All Certificates:
https://drive.google.com/drive/folders/1JO63AnLyrIuzTVrwTjdNV_WxbFDBPqAY

🔗 All Badges :
https://www.credly.com/users/geet-prajapati

---

## 📫 Connect With Me

- 📧 Email: prajapatigeet12@gmail.com
- 💼 LinkedIn: https://www.linkedin.com/in/geetprajapati12
- 📍 Ahmedabad, Gujarat

---

## ⚡ Fun Fact

I enjoy combining cybersecurity, testing, and development to build secure and reliable digital experiences.

⭐ Thanks for visiting my profile!
